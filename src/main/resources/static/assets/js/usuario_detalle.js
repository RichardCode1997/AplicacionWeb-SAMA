$(document).ready(function() {
  ...
  var urlParams = new URLSearchParams(window.location.search);
    ...
    $('.profile-card h3').empty();
      ...
      $.ajax({
        url: 'http://localhost:8080/roles', // Reemplaza 'ruta_del_servidor' por la URL de tu endpoint AJAX
        type: 'GET',
//        dataType: 'json',
...
// Recorre los datos recibidos y agrega las opciones al elemento select
            $('#nombreRol').append('<option value="' + roles.idRol + '">' + roles.nombreRol  + '</option>');
          });
        },
        error: function() {
...
//      console.log(data)
        ...
        $('.profile-card h3').text(data.rol.nombreRol);
